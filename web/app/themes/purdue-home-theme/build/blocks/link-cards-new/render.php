...
?>
<div <?= $id; ?> class="purdue-home-link-cards <?= $attributes['className']; ?>">
...
?>
    ...
    <h2 class="tagged-header<?= $headerClass; ?>"><?= $attributes['header']; ?></h2></div>
<?php } ?>
</div>
...
?>
                ...
                foreach( $results as $card){
                   ...
                   ?>
                   <?= card($card, $attributes["cardType"],$attributes["inculdeDesc"],$attributes["includeThumb"], $attributes["addPostTypeTag"],$attributes["addTaxTag"],$attributes["cardBackground"],$attributes["horizontal"]);?>                   
                    </div>
               <?php } ?>
                </div></div>
                <?php
        ...
        if($attributes['layout']=="grid"){
            ...
            foreach( $attributes['cards'] as $card){
                ...
                ?>
                <div class="<?= $columnClass; ?>">
                <?= card($card, $attributes["cardType"],$attributes["inculdeDesc"],$attributes["includeThumb"], true, true,$attributes["cardBackground"],$attributes["horizontal"]);?></div>
                <?php } ?>
            </div></div>
        <?php
        }else{
        ...
        ?>
            ...
            ?>
             <div class="glide__slide">
             <?= card($card, $attributes["cardType"],$attributes["inculdeDesc"],$attributes["includeThumb"],true, true,$attributes["cardBackground"],$attributes["horizontal"]);?>  
             </div>
             <?php } ?>
            </div></div>
            <?php
...
if($attributes["linkURL"]!=""){        
?>
   <div class="button-container mobile-show"><a class="purdue-home-button<?= $buttonClass;?>" href="<?= $attributes["linkURL"];?>" <?= $target1;?>><?= trim($attributes["linkText"]); ?></a></div>
